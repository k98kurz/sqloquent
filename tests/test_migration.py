...
class TestMigration(unittest.TestCase):
    ...
    def test_Table_create(self):
        ...
        assert sql[1] == (
            'create unique index if not exists udx_things_id_name on "things" '
            '("id", "name")'
        ), sql[1]
        ...
        assert sql[0] == (
            ...
            '"name" text, "parts" numeric, "fraction" real, "data" blob)'
        ), f"'{sql[0]}' is wrong"
    ...
    def test_Migration_up_and_down_e2e(self):
        ...
        expected = (
            'begin;\ncreate table if not exists "things" ("id" integer, '
            '"name" text, "data" blob default (x\'313233\'), "is_active" boolean '
            'default True);\n'
            'create unique index if not exists udx_things_id on "things" ("id");\n'
            'create index if not exists idx_things_name on "things" ("name");\n'
            'commit;'
