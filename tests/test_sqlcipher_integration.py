from context import tools, cipher_classes
from decimal import Decimal
from genericpath import isdir, isfile
from integration_vectors import cipher_models as models, cipher_models2 as models2
...
MODELS_PATH = 'tests/integration_vectors/cipher_models'
...
class TestIntegration(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        """Monkey-patch the file path and encryption key."""
        # set encryption key
    ...
    def test_integration_e2e(self):
        ...
        for name in names:
            src = tools.make_migration_from_model_path(
                ...
                ctx=('SqlcipherContext', 'sqloquent.sqlcipher')
            )
            if name == 'Account':
        ...
        tools.automigrate(MIGRATIONS_PATH, DB_FILEPATH, use_cipher=True)
    ...
    def table_exists(self, name: str) -> bool:
        ...
        #return len(self.getDbCursor().execute(q).fetchall()) > 0
        with sqlcipher3.connect(DB_FILEPATH) as conn:
            conn.execute(f"PRAGMA key = '{ENCRYPTION_KEY}'")
            return len(conn.execute(q).fetchall()) > 0
        ...
        with sqlcipher3.connect(DB_FILEPATH) as conn:
            ...
            for name in names:
                q = f"select name from sqlite_master where type='table' and "\
                    f"name='{name}'"
                if len(conn.execute(q).fetchall()) == 0:
                    return False
        return True
